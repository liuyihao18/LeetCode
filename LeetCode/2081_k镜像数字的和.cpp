// 2081_k¾µÏñÊý×ÖµÄºÍ.cpp
#include "custom.h"
ustd

class Solution {
    pair<long long, long long> generatePalindrome(int half) {
        string s = to_string(half);
		string rev = s;
		reverse(rev.begin(), rev.end());
        string p1 = s.substr(0, s.size() - 1) + rev, p2 = s + rev;
        return { stoll(p1), stoll(p2) };
    }
    bool isKMirror(long long num, int k) {
        string s;
        while (num > 0) {
            s += static_cast<char>(num % k);
            num /= k;
        }
        string rev = s;
        reverse(rev.begin(), rev.end());
...
public:
    long long kMirror(int k, int n) {
        ...
        int count = 0;
        long long sum = 0;
        for (int i = 1; count < n && i < 1000000; i++) {
            pair<long long, long long> palindrome = generatePalindrome(i);
            v1.push_back(palindrome.first);
            v2.push_back(palindrome.second);
            long long curr = 0;
